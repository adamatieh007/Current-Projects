...
vector<double> mean_diff_sampling_distribution(
  ...
  vector <double> computedValue = {};

  for (int i = 0; i < 1000; i++)
  {
    vector <double> bootA = bootstrap_resample(data_A, i);
    vector <double> bootB = bootstrap_resample(data_B, i);
    ...
    computedValue.push_back(difference);
...
pair<double, double> confidence_interval(vector<double> v, double width) {
  ...
  double lowbound = (1 - width) / 2;
  double highbound = 1 - lowbound;

  double lower = percentile(v, lowbound);
  double upper = percentile(v, highbound);
